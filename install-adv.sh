...
if grep -q "avoid_warnings=2" "$File";
        then
                sed -i '/avoid_warnings=2/c\avoid_warnings=1' "$File";
fi
...
if [ -e $script ];
	then
		echo "Script shutdown-nespi.py already exists. Updating..."
                rm -r $script
		rm -r $tone
		rm -r $killes
		wget "https://raw.githubusercontent.com/mafe72/Retroflag-NESPi-Front-Panel-Control-Board/master/scripts/shutdown-nespi.py"
		...
		wget "https://raw.githubusercontent.com/mafe72/Retroflag-NESPi-Front-Panel-Control-Board/master/scripts/shutdown-nespi.py"
                ...
                wget "https://raw.githubusercontent.com/mafe72/Retroflag-NESPi-Front-Panel-Control-Board/master/scripts/killes.sh"
                chmod a+x killes.sh
		echo "Installing Kill ES Service...."
                cd /etc/systemd/system/
                wget "https://raw.githubusercontent.com/mafe72/Retroflag-NESPi-Front-Panel-Control-Board/master/scripts/killes.service"
		sudo systemctl enable killes
...
if grep -q "sudo python \/opt\/RetroFlag\/shutdown-nespi.py \&" "$RC";
	then
		...
		sed -i -e "s/^exit 0/sudo python \/opt\/RetroFlag\/shutdown-nespi.py \&\n&/g" "$RC"
...
echo "Retroflag NESPi Advance Front Panel Control Switch installation done. Will now reboot after 3 seconds."
sleep 4
sudo reboot
#-----------------------------------------------------------
